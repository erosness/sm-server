// stolen from http://www.musicdsp.org/files/biquad.c
// see http://www.musicdsp.org/showone.php?id=64
...
biquad *BiQuad_new(int type, smp_type dbGain, smp_type freq, smp_type srate, smp_type Q)
{
    biquad *b;
    ...
    sn = sin(omega);
    cs = cos(omega);
    // alpha changed from: //sinh(M_LN2 /2 * bandwidth * omega /sn);
    alpha = sn / (2.0 * Q);
    beta = sqrt(A + A);
...
// helper function to convert from biquad.c to sigma-notation
// as well as helpers for scheme bindings.

// TODO: add type
int fill_biquad_coefficients(double* dest, double gain, double f0, double Fs, double Q) {

  biquad* bq = BiQuad_new(PEQ, gain, f0, Fs, Q);
  if (bq) {
    dest[0] = bq->a0;
    dest[1] = bq->a1;
    dest[2] = bq->a2;
